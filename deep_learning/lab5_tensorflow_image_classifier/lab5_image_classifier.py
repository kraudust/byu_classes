...
h0 = conv(input_image,name='conv1')
h1 = conv(h0,name='conv2')
h2 = conv(h1,stride = 2, name='conv3')
h2_shape = h2.get_shape().as_list()
h2_flat = tf.reshape(h2, [h2_shape[1]*h2_shape[2]*h2_shape[3], -1])
fc0 = fc(h2_flat,out_size = 25,name='fc1')
score = fc(fc0, out_size = 10,is_output = True, name = 'fc2')
...
for i in range(len(labels_train)):
    ...
    sess.run(train_weights, feed_dict = {input_image:im, lab:labels_train[i,0]})
    ss = sess.run(merged, feed_dict = {input_image:im, lab:labels_train[i,0]})
    ...
    if i % 100.0 == 0: #check accuracy every 100 training images
        ...
        for j in range(200): #test accuracy on 200 random images from the test set
            acc_i = sess.run(acc, feed_dict = {input_image: reshape_cifar_image(features_test[test_indices[j],:]), lab: labels_test[test_indices[j],0]})
