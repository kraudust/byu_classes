function [mu_t, sigma_t, Kt] = UKF_mult_meas(mu_tm1, sigma_tm1, u_tm1, z_t, m, sigma_r, sigma_phi,Ts,alpha_u)
    L = size(mu_tm1,1) + size(u_tm1,1) + 2;
    n = L;
    vt = u_tm1(1);
    omegat = u_tm1(2);
    ...
    mu_a_tm1 = [mu_tm1; zeros(size(u_tm1,1),1); zeros(2,1)]; %augmented state
    ...
    for i = 1:size(m,1)
        mx = m(i,1);
        my = m(i,2);
        z_ti = [z_t(i); z_t(i+size(m,1))];
        % Redraw sigma points------------------------------------------------------------
        sigma_bar_t_a = [...
            sigma_bar_t,                              zeros(size(sigma_tm1,1),size(Mt,2)),    zeros(size(sigma_tm1,1),size(Qt,2));...
            zeros(size(Mt,1),size(sigma_tm1,2)),    Mt,                                     zeros(size(Mt,1),size(Qt,2));...
            zeros(size(Qt,1),size(sigma_tm1,2)),    zeros(size(Qt,1),size(Mt,2)),           Qt]; %augmented covariance
        R = chol(sigma_bar_t_a); % matrix square root
        R = R'; % get the lower triangular instead of the upper triangular
        alpha = .1; %scaling parameter to determine how spread out sigma points are
        kappa = 3; %scaling parameter to determine how spread out sigma points are
        lamda = alpha^2 * (n + kappa) - n;
        gamma = sqrt(n+lamda);
        mu_bar_t = [mu_bar_t; zeros(size(u_tm1,1),1); zeros(2,1)]; %augmented state
        chi_a_t = [mu_bar_t, repmat(mu_bar_t,1,L) + gamma*R, repmat(mu_bar_t,1,L) - gamma*R];
        chi_x_t = chi_a_t(1:3,:);
        chi_z_t = chi_a_t(6:7,:);
        %----------------------------------------------------------------------------------

        z_bar_t = [((mx - chi_x_t(1,:)).^2 + (my - chi_x_t(2,:)).^2).^0.5 + chi_z_t(1,:);...
            atan2(my - chi_x_t(2,:), mx - chi_x_t(1,:)) - chi_x_t(3,:) + chi_z_t(2,:)];
        z_hat_t = zeros(2,1);
        for i = 1:2
        ...
        for i = 1:2*L+1
           sigma_xz_t = sigma_xz_t + wc(i)*(chi_x_t(:,i) - mu_bar_t(1:3))*(z_bar_t(:,i) - z_hat_t).';
        end

        %Update mean and covariance
        Kt = sigma_xz_t/St; %Kalman Gain
        mu_bar_t = mu_bar_t(1:3) + Kt*(z_ti-z_hat_t); %measurement update
        sigma_bar_t = sigma_bar_t - Kt*St*Kt.'; %update measurement covariance
    end
